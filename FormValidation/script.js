document.addEventListener("DOMContentLoaded", () => {
  ...
  const validName = (fname) => fname.trim().length >= 3;
  const validFamily = (lname) => lname.trim().length >= 3;
  const validAge = (age) => age >= 18 && age <= 100;
  const validEmail = (email) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
  const validMobile = (mobile) => /^09\d{9}$/.test(mobile);
